...
alias grep='rg --line-number --color=always'
...
brightness() {
    LEVEL="$1"
    if (($(echo "$LEVEL <= 0.0" | bc -l))); then
        echo "Brightness cannot be below 0"
    elif (($(echo "$LEVEL > 1.5" | bc -l))); then
...
function rgfzf {
    ...
    command rg --color=always --line-number --no-heading --smart-case "$@" "$search" |
        command fzf -d':' --ansi \
            --preview "command bat -p --color=always {1} --theme=base16 --highlight-line {2}" \
